...
from config import MAX_CLARIFY_ROUNDS, INTERACTIVE, MODEL, MAX_TOKENS_CLARIFY, language_rule
...
{language_rule("LANGUAGE RULE: Detect the primary language of the source documents. If all documents are in the same language, write ALL follow-up question text in that language. If documents are in multiple languages, use English.")}
The JSON keys (sufficient, follow_up_questions) are always in English. Only the question VALUES must be in the detected/forced language.
...
def run_clarifying_loop(folder: str) -> dict:
    ...
    for round_num in range(MAX_CLARIFY_ROUNDS):
        ...
        if response.stop_reason == "max_tokens":
            raise RuntimeError(
                f"[CLARIFY] Response truncated at round {round_num + 1} — "
                f"increase max_tokens.clarify in config.yaml (currently {MAX_TOKENS_CLARIFY})."
            )
